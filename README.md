...
六支队通用无人机一图勘察数字模型。上传一张事故现场航拍图后，可自动识别车辆和道路，人工校对后生成可交互的三维现场 HTML。
...
- **AI 分析（可选）**：配置 DeepSeek 密钥后，可按交警专家角色生成责任分析与报告发言。
...
环境就绪后可运行 `build.bat`，产物在 `release/TrafficAccident3D/`。打包时会清空 API 密钥，但会带上设置文件和 YOLO 权重。
